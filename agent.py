...
class MonteAgent(Agent):
    ...
    def __init__(self, env: Environment, time_limit: int, max_depth: int, explore_exploit: float):
        ...
        state = get_world_state_hash(
            env.get_player_location(), valid_actions, False)
        self.root = Transposition_Node(
            state, None, None, valid_actions, self.transposition_table, score)
    ...
    def take_action(self, env: Environment, history: list) -> str:
        ...
        while(seconds_elapsed < self.time_limit):
            ...
            new_node, path = tree_policy(
                self.root, env, self.reward, self.transposition_table)
