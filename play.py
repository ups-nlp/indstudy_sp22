...
def play_game(agent: Agent, env: Environment, num_steps: int):
    ...
    while num_steps != 0 and not done:
        ...
        if num_times_called > 0 and num_times_called % 10 == 0:
            ...
            print(
                f'\tOf those {num_times_called} steps, how many were valid? {m}')
            print(
                f'\tOf those {num_times_called} steps, how many changed your location? {num_location_changes}')
            print(
                f'\tHow long to call take_action() {num_times_called} times? {seconds}')
            print(
                f'\tAverage number of seconds spent in take_action() {seconds/num_times_called}')
            print(
                f'\tNumber of MCTS iterations performed overall? {num_iters}')
            print(
                f'\tAverage number of MCTS iters per call to take_action()? {num_iters/num_times_called}')
            print()
    ...
    if args.agent == 'random':
        ...
        if args.mcts_time is None:
            ...
            ai_agent = MonteAgent(env, args.mcts_time,
                                  args.mcts_depth, args.explore_exploit)
    else:
        ai_agent = RandomAgent()
