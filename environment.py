...
class ChamberEnvironment(Environment):
    ...
    def get_state(self):
        """Returns the internal game state"""
        state = {'moves': list(self.moves), 'game_over': self.is_game_over,
...
class Chambers4Environment(Environment):
    ...
    losing_obs = "You failed to open the door. You lose."
    winning_obs = "You open the door. You have won!"
    actions = ["n", "e", "s", "w", "open",
               "fight", "look", "take key", "drop key"]
    reward_key = 5
    reward_door = 10
    ...
    def get_state(self):
        """Returns the internal game state"""
        state = {'moves': deepcopy(self.moves), 'game_over': self.is_game_over,
                 'victory': self.is_victory, 'last_obs': self.last_obs, 'reward': self.reward}
        return state
